...
contract BridgeAdapter is IBridgeAdapter, Ownable, ReentrancyGuard, Pausable {
    ...
    modifier validChainId(uint256 chainId) {
        require(
            chainId == 1 || chainId == 56 || chainId == 295,
            "Unsupported chain"
        ); // Ethereum, BSC, Hedera
        _;
    }
    ...
    function createTokenMapping(
        ...
        require(
            htsToken != address(0) && erc20Token != address(0),
    ...
    function completeBridgeOperation(
        ...
        require(
            operation.status == BridgeStatus.Pending,
            "Operation not pending"
        );
        require(
            block.timestamp <= operation.timestamp + OPERATION_TIMEOUT,
            "Operation expired"
        );
    ...
    function setBridgeLimits(
        ...
        require(
            dailyLimit >= singleOpLimit,
            "Daily limit must be >= single op limit"
        );
     ...
     */
    function deactivateTokenMapping(
        address token,
        uint256 chainId
    ) external onlyOwner {
        bytes32 mappingKey = _getMappingKey(token, chainId);
    ...
    function _findMappingByERC20(
        ...
        revert("Token mapping not found or inactive");
    }
    ...
    function _validateBridgeLimits(
        ...
        require(
            dailyAmount + amount <= bridgeLimits[token],
