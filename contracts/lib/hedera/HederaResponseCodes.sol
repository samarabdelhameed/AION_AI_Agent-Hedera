...
library HederaResponseCodes {
    ...
    int32 internal constant TRANSACTION_REQUIRES_ZERO_TOKEN_BALANCES = 195; // An attempted operation is not allowed on an account that is frozen for the token
    int32 internal constant ACCOUNT_IS_TREASURY = 196; // Account is the treasury for a unique token and cannot be deleted or auto-renewed
    ...
    int32 internal constant UNRESOLVABLE_REQUIRED_SIGNERS = 206; // The required signers for a scheduled transaction cannot be resolved, for example because they require the use of unknown hedera functionality
    ...
    int32 internal constant NODE_CAPACITY_NOT_SUFFICIENT_FOR_OPERATION = 217; // Given the network size in the address book, the node-level capacity for an operation would not support the desired throughput
    ...
    int32 internal constant INVALID_THROTTLE_DEFINITIONS = 222; // The new throttle definitions system file were valid protobuf but not valid throttle definitions
    ...
    int32 internal constant TOKEN_NOT_ASSOCIATED_TO_FEE_COLLECTOR = 235; // Any of the token Ids in customFees are not associated to the feeCollector
    ...
    int32 internal constant CURRENT_TREASURY_STILL_OWNS_NFTS = 250; // The treasury for a unique token cannot be changed until the token owner owns no NFTs
    int32 internal constant ACCOUNT_STILL_OWNS_NFTS = 251; // An account cannot be dissociated from a unique token if the account owns NFTs for the token
    ...
    int32 internal constant SERIAL_NUMBER_LIMIT_EXCEEDED = 260; // Currently no more than 4,294,967,295 NFTs may be minted for a given unique token type
    ...
    int32 internal constant REQUESTED_NUM_AUTOMATIC_ASSOCIATIONS_EXCEEDS_ASSOCIATION_LIMIT = 264; // Cannot set the number of automatic associations for an account more than the maximum allowed token associations tokens.maxPerAccount.
    int32 internal constant TOKEN_IS_PAUSED = 265; // Token is paused and not associated to Account
    ...
    int32 internal constant INVALID_ALIAS_KEY = 282; // An alias used in a CryptoTransfer transaction is not the serialization of a primitive Key message--that is, a Key with a single Ed25519 or ECDSA_Secp256k1 public key and no unknown protobuf fields.
    ...
    int32 internal constant CANNOT_APPROVE_FOR_ALL_FUNGIBLE_COMMON = 291; // [Deprecated] SetApprovalForAll is not supported on fungible tokens.
    ...
    int32 internal constant AMOUNT_EXCEEDS_ALLOWANCE = 293; // The transfer or approval operation would cause the spender's allowance with the owner to become negative.
    int32 internal constant MAX_ALLOWANCES_EXCEEDED = 294; // The payer account cannot grant approvals on the owner's behalf as it would exceed the maximum account limit of approved allowances.
    int32 internal constant EMPTY_ALLOWANCES = 295; // All allowances on an account cannot be granted to 0.0.0.
    int32 internal constant FUNGIBLE_TOKEN_IN_NFT_ALLOWANCES = 296; // [Deprecated] Fungible common tokens cannot be used in NFT allowances operations.
    int32 internal constant NFT_IN_FUNGIBLE_TOKEN_ALLOWANCES = 297; // [Deprecated] Non fungible tokens cannot be used in fungible token allowances operations.
    int32 internal constant INVALID_ALLOWANCE_OWNER_ID = 298; // The allowance owner account id is invalid or does not exist.
    int32 internal constant INVALID_ALLOWANCE_SPENDER_ID = 299; // The allowance spender account id is invalid or does not exist.
    int32 internal constant REPEATED_SERIAL_NUMS_IN_NFT_ALLOWANCES = 300; // [Deprecated] Repeated serial numbers are not allowed in nft allowances.
    int32 internal constant FUNGIBLE_COMMON_ALLOWANCE_OWNER_CHANGE_NOT_ALLOWED = 301; // [Deprecated] Fungible common allowances owner cannot be changed using NFT allowances transactions. Use fungible token allowances transactions instead.
    int32 internal constant NO_ALLOWANCE_FOR_NFT = 302; // [Deprecated] No NFT serial numbers are allowed in Fungible common allowances transactions.
}