...
import "../src/strategies/StrategyCompound.sol";
import "../src/AIONVault.sol";

contract StrategyCompoundTest is Test {
    StrategyCompound public strategy;
    ...
    // ========== Constants ==========
    address constant CTOKEN_ADDRESS =
    ...
    function setUp() public {
        ...
        strategy = new StrategyCompound(CTOKEN_ADDRESS, UNDERLYING_TOKEN);
    ...
    function testSetStrategyByAIAgent() public {
        address newStrat = address(
            new StrategyCompound(CTOKEN_ADDRESS, UNDERLYING_TOKEN)
        );
        vm.prank(aiAgent);
    ...
    function testStrategyCompoundBasicFunctions() public {
        // Test دوال StrategyCompound الأساسية
        assertEq(
            strategy.vault(),
            address(vault),
        ...
        assertEq(
            strategy.getCTokenAddress(),
            CTOKEN_ADDRESS,
            "cToken address should be correct"
        );
        assertEq(
        ...
        assertEq(
            ...
            700,
            "APY should be 7% (700 basis points)"
        );
        assertEq(
            strategy.strategyName(),
            "StrategyCompoundLending",
    ...
    function testVaultStatsAndInfo() public {
        ...
        ) = strategy.getCompoundStats();

        assertEq(
            cTokenAddress,
            CTOKEN_ADDRESS,
            "cToken address in stats should be correct"
        );
        assertEq(
            tokenAddress,
            UNDERLYING_TOKEN,
        ...
        assertEq(estimatedYield, 700, "Estimated yield should be 7%");
        assertEq(
            strategyTypeName,
            "Compound Lending",
    ...
    function test_RevertWhen_WithdrawMoreThanBalance() public {
        ...
        vm.prank(user1);
        // Try to withdraw without deposit - should revert with any error message
        vm.expectRevert();
    ...
    function testRealCompoundIntegration_DepositYieldClaim() public {
        // Test تكامل حقيقي مع Compound
        vm.deal(user1, 5 ether);
        vm.prank(user1);
        vault.deposit{value: 2 ether}();
    ...
    function testGasEfficiency() public {
        ...
        vm.warp(block.timestamp + 7 days);
        console.log("Yield claim would be processed");
    ...
    function testCompoundLendingRates() public {
        // Test معدلات الإقراض في Compound
        vm.deal(user1, 5 ether);
        vm.prank(user1);
        vault.deposit{value: 1 ether}();

        // Compound typically has lower rates than other protocols
        uint256 yield = strategy.getYield(user1);
        console.log("Compound yield:", yield);

        // In test environment, yield might be 0 due to mock implementation
        // This is acceptable for testing purposes
        if (yield > 0) {
            console.log("Compound generated yield:", yield);
        } else {
    ...
    function testCompoundStability() public {
        // Test استقرار Compound
        vm.deal(user1, 10 ether);
        vm.prank(user1);
