...
contract BridgeServiceIntegrationTest is Test {
    ...
    function setUp() public {
        ...
        bridgeAdapter = new BridgeAdapter(owner, owner, address(htsTokenManager));
        
        // Deploy Bridge Service Integration
        integration = new BridgeServiceIntegration(owner, address(bridgeAdapter));
    ...
    function testAddBridgeService() public {
        ...
        BridgeServiceIntegration.ServiceConfig memory config = integration.getServiceConfig(address(hashportService));
        assertEq(address(config.service), address(hashportService));
        assertTrue(config.active);
        assertEq(config.priority, 100);
        assertEq(uint256(config.serviceType), uint256(BridgeServiceIntegration.BridgeServiceType.Hashport));
    ...
    function testAddMultipleBridgeServices() public {
        ...
        address[] memory bscServices = integration.getServicesForChain(BSC_CHAIN_ID);
    ...
    function testRemoveBridgeService() public {
        ...
        assertTrue(integration.getServiceConfig(address(hashportService)).active);
        
        // Remove service
        integration.removeBridgeService(address(hashportService));
        
        // Verify service was removed
        assertFalse(integration.getServiceConfig(address(hashportService)).active);
    ...
    function testSetPreferredServiceByChain() public {
        ...
        integration.setPreferredServiceByChain(BSC_CHAIN_ID, address(hashportService));
        
        // Verify preferred service
        assertEq(integration.preferredServiceByChain(BSC_CHAIN_ID), address(hashportService));
    ...
    function testEstimateBridgeFee() public {
        ...
        (uint256 fee, address service) = integration.estimateBridgeFee(BSC_CHAIN_ID, 200000, payload);
    ...
    function testGetServicesForChain() public {
        ...
        address[] memory ethServices = integration.getServicesForChain(ETH_CHAIN_ID);
        assertEq(ethServices.length, 1);
        assertEq(ethServices[0], address(layerZeroService));
        
        // Test unsupported chain
        address[] memory hederaServices = integration.getServicesForChain(HEDERA_CHAIN_ID);
    ...
    function testServiceSupportsChain() public {
        ...
        assertTrue(integration.serviceSupportsChain(address(hashportService), BSC_CHAIN_ID));
        
        // Test unsupported chain
        assertFalse(integration.serviceSupportsChain(address(hashportService), ETH_CHAIN_ID));
    ...
    function testGetServiceStats() public {
        ...
        assertTrue(isActive);
        assertEq(uint256(serviceType), uint256(BridgeServiceIntegration.BridgeServiceType.Hashport));
        
        vm.stopPrank();
    }
    
    function testFailAddInvalidService() public {
        vm.startPrank(owner);
    ...
    function testFailAddDuplicateService() public {
        vm.startPrank(owner);
    ...
    function testFailSetPreferredServiceUnsupportedChain() public {
        vm.startPrank(owner);
        ...
        integration.setPreferredServiceByChain(ETH_CHAIN_ID, address(hashportService));
